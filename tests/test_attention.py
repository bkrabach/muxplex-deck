...
- Tier 2: the active session, when not already in tier 1.
- Tier 3: everything else, ordered by `bell.last_fired_at` descending, with
  never-belled sessions sorting last and stable order preserved among ties.
- Regression guard: tier 3 must follow `bell.last_fired_at`, not
...
def test_tier1_needs_attention_sessions_first_newest_bell_first() -> None:
    ...
    result = apply_attention_sort([a, b, c], active_session=None)

    names = [s.name for s in result]
    assert names == ["b", "a", "c"]


def test_tier2_active_session_placed_after_tier1_when_not_already_there() -> None:
    bell_session = _session("bell", last_fired_at=100.0, seen_at=None, unseen_count=1)
    active = _session("active")
    other = _session("other")

    result = apply_attention_sort(
        [other, active, bell_session], active_session="active"
    )
...
def test_active_session_already_in_tier1_is_not_duplicated() -> None:
    bell_and_active = _session(
        "bell-active", last_fired_at=100.0, seen_at=None, unseen_count=1
    )
    other = _session("other")

    result = apply_attention_sort(
        [other, bell_and_active], active_session="bell-active"
    )
...
def test_tier3_orders_by_bell_last_fired_at_descending() -> None:
    ...
    result = apply_attention_sort(
        [never_belled, old_bell, recent_bell], active_session=None
    )
...
def test_tier3_regression_guard_follows_bell_not_last_activity_at() -> None:
    ...
    result = apply_attention_sort(
        [fresh_activity_old_bell, old_activity_fresh_bell], active_session=None
    )
...
def test_tier3_never_belled_sessions_sort_after_ever_belled_sessions() -> None:
    ...
    result = apply_attention_sort(
        [never_belled_1, ever_belled, never_belled_2], active_session=None
    )
...
def test_tier3_ties_preserve_incoming_base_order_stable_sort() -> None:
    first = _session("first")
    second = _session("second")
    third = _session("third")

    result = apply_attention_sort([first, second, third], active_session=None)

    names = [s.name for s in result]
    assert names == ["first", "second", "third"]


def test_sessions_not_in_input_list_are_never_added() -> None:
    other = _session("other")

    result = apply_attention_sort([other], active_session="missing-from-view")
